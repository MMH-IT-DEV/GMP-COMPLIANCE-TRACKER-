...
export const DiscussionPanel: React.FC = () => {
    ...
    const getAvatarColor = (name: string) => {
        ...
        const colors = [
            ...
            'bg-warning', 'bg-error', 'bg-slate-500', 'bg-indigo-500'
        ];
    ...
    return (
        <div
            ref={panelRef}
            className={`fixed top-0 right-0 h-full bg-[#1a1d21] border-l border-white/10 z-50 flex flex-col shadow-2xl transition-all duration-300 ease-in-out`}
            style={{ width: `${width}px` }}
        >
            {/* Resize handle */}
            <div
                className="absolute top-0 left-0 w-1.5 h-full cursor-ew-resize hover:bg-olive/40 transition-colors z-[60]"
                onMouseDown={startResizing}
            />

            {/* Header */}
            <div className="px-6 py-4 border-b border-white/10 flex justify-between items-center bg-[#1a1d21]">
                <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-base text-white truncate pr-4">
            ...
            {/* Messages */}
            <div className="flex-1 overflow-y-auto bg-[#1a1d21] py-4">
                {messages.length === 0 ? (
                    ...
                    <div className="space-y-1">
                        {messages.map((msg, idx) => {
                            ...
                            return (
                                <div
                                    key={msg.id}
                                    className={`group px-6 py-1.5 hover:bg-white/[0.03] transition-colors ${showAvatar ? 'mt-4' : ''}`}
                                >
                                    <div className="flex gap-4">
                                        {showAvatar ? (
                                            <div className={`w-9 h-9 rounded-lg ${getAvatarColor(msg.user_name)} flex items-center justify-center text-white text-sm font-bold shrink-0 shadow-lg`}>
                                                {getInitials(msg.user_name)}
                                            </div>
                                        ) : (
                                            <div className="w-9 shrink-0 flex items-center justify-center">
                                                <span className="text-[9px] text-white/0 group-hover:text-white/30 transition-colors">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            {showAvatar && (
                                                <div className="flex items-baseline gap-2 mb-0.5">
                                                    <span className="font-black text-[15px] text-white hover:underline cursor-pointer">
                                                        {msg.user_name}
                                                    </span>
                                            ...
                                            <div className="text-[15px] text-white/90 leading-normal break-words">
                                                {msg.message}
                                            </div>
                                        </div>
                                    </div>
            ...
            {/* Input area */}
            <div className="px-6 pb-6 pt-2 bg-[#1a1d21]">
                <div className="border border-white/20 rounded-xl bg-white/[0.03] overflow-hidden focus-within:border-white/40 transition-colors group">
                    <div className="flex flex-col">
                        {/* Toolbar placeholder */}
                        <div className="px-3 py-2 flex gap-2 border-b border-white/5 text-white/30">
                            <div className="w-5 h-5 rounded hover:bg-white/5 cursor-pointer flex items-center justify-center"><b>B</b></div>
                            <div className="w-5 h-5 rounded hover:bg-white/5 cursor-pointer flex items-center justify-center italic">I</div>
                            <div className="w-5 h-5 rounded hover:bg-white/5 cursor-pointer flex items-center justify-center line-through">S</div>
                            <div className="mx-1 border-r border-white/5 h-4 my-auto"></div>
                            <div className="w-5 h-5 rounded hover:bg-white/5 cursor-pointer flex items-center justify-center opacity-70">🔗</div>
                        </div>
                        ...
                        <form onSubmit={handleSendMessage} className="relative p-1">
                            <textarea
                                ...
                                className="w-full bg-transparent border-none px-4 py-3 text-[15px] text-white placeholder:text-white/20 focus:ring-0 outline-none resize-none h-24"
                            />
                            <div className="flex justify-between items-center px-3 pb-2">
                                <div className="flex gap-2">
                                    <button type="button" className="p-1.5 text-white/30 hover:text-white/60 hover:bg-white/5 rounded transition-all">
                                ...
                                <button
                                    ...
                                    className={`p-2 rounded-lg transition-all ${newMessage.trim() ? 'bg-success text-white shadow-lg' : 'bg-white/5 text-white/20'}`}
                                >
